    ...
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box)
{
    ...
    fuse_conv_batchnorm(net);
    calculate_binary_weights(net);
    if (net.layers[net.n - 1].classes != names_size) {//不走
        printf(" Error: in the file %s number of names %d that isn't equal to classes=%d in the file %s \n",
            name_list, names_size, net.layers[net.n - 1].classes, cfgfile);
    ...
    KinectSensorC* pSensorC = NULL;
    darknetKinectSensorOpenC(&pSensorC);
    KinectColorFrameReaderC* pColorFrameReaderC = NULL;
    pColorFrameReaderC = darknetKinectOpenColorReaderC(&pSensorC);
    KinectDepthFrameReaderC* pDepthFrameReaderC = NULL;
    pDepthFrameReaderC = darknetKinectOpenDepthReaderC(&pSensorC);
    KinectCoordinateMapperC* pCoorMapperC = NULL;
    pCoorMapperC = darknetKinectOpenMapperC(&pSensorC);

#endif // IMG_FROM_KINECT
    /*Kinect部分over*/

    while (1) {
        double whileTime = get_time_point();
        if (filename) {//test时 不输入文件名，不走这里
            printf("********************filename\n\n\n");
        ...
        /*魔改image im部分*/
#ifndef IMG_FROM_KINECT
        image im = load_image(input, 0, 0, net.c);//这句话应该是要改掉的
#endif // IMG_FROM_KINECT
#ifdef IMG_FROM_KINECT
        ...
        double showTime = get_time_point();
        if (!dont_show) {
        ...
        double freeTime = get_time_point();
        free_detections(dets, nboxes);
        free_image(im);
        free_image(sized);
